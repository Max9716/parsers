...
data = {
    ...
    "scope": "SITE_WIDGET",
    "referer": "https://gk-crocus.webtm.ru"
}
...
def floor(x, y):
    url = f'https://pb19835.profitbase.ru/api/v4/json/floor?houseId={x}'
...
def poisk(y):
    try:
        ...
        flat1["type_room"] = y["propertyType"]+str(y["rooms_amount"])
        flat1["price"] = y["price"]["value"]
        flat1["price_base"] = y["price"]["value"]
        flat1["area"] = y["area"]["area_total"]
        flat1["areaH"] = y["area"]["area_living"]
        flat1["areaK"] = y["area"]["area_kitchen"]
        flat1["status"] = y["status"]
        flat1["decor"] = y["attributes"]["facing"]
        try:
...
for i in range(0, 1500, 75):
    url = f'https://pb19835.profitbase.ru/api/v4/json/property?isHouseFinished=0&status%5B0%5D=AVAILABLE&limit=75&offset={i}&full=true&returnFilteredCount=true'
    response = requests.get(url, headers=headers)
    response.encoding = 'utf-8'
    try:
...
with open("crokus.xml", "wb") as myfile:
    myfile.write(mydata)
